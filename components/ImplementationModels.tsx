        ...
        <div className="grid md:grid-cols-2 gap-8">
          {implementationModels.map((model, idx) => (
            <div 
              key={idx}
              className={cn(
                "card-base bg-aegrix-surface border-white/3 p-10 hover:border-aegrix-cyan/20 transition-all duration-700",
                inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
              )}
            ...
            >
              <div className="flex items-center gap-4 mb-8">
                ...
                <h3 className="heading-md text-white">{model.title}</h3>
              </div>
